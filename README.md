# 👋 Hi, I'm 최찬식 (Chansik Choi)

공통 컴포넌트 설계와 구조화를 통해  
재사용성과 유지보수성을 개선해 왔으며, 프론트엔드 개발자로의 전환을 지원하는 5년 차 퍼블리셔 최찬식입니다.

4년 이상 다양한 프로젝트에서 UI를 설계하며  
반복 작업을 줄이고, 일관된 기준으로 확장 가능한 구조를 만드는 데 집중해왔습니다.

---

## 💼 About Me

- **Experience**: 2021.06.28 ~ Present (약 5년)
- **Company**: 프레임아웃 (Frameout)

### 🔹 What I Focus On
- 공통 컴포넌트 설계를 통한 재사용성과 유지보수성 개선  
- Wrapper 계층과 도메인 컴포넌트 분리를 통한 구조 설계  
- 반복되는 UI 및 설정 작업을 줄이기 위한 구조화  
- 사용자 입력, 상태 변화에 따른 UI 분기 설계  

### 🔹 Leadership
- 퍼블리싱 파트장 (2명 리딩)  
- UI 컴포넌트 가이드 및 협업 기준 정의  
- 코드 리뷰 및 프로젝트 진행 관리  

---

## 📚 Side Project

### 1. 권한 기반 인터뷰어 피드백 보드
- **Project Link**: [Next.js + Supabase로 만든 권한 기반 피드백 보드](https://next-js-page-router-fetch-api.vercel.app/)
- **Tech & Library**: Next.js, TypeScript, Supabase, React Hook Form

**What I Did**
- Supabase Auth 기반 인증 (로그인, 회원가입, 권한 관리)  
- 사용자 권한에 따른 UI 및 데이터 접근 분기  
- 피드백 작성/수정/삭제 및 승인/반려/재검토 흐름 설계  
- 상태 기반 UI 제어 및 액션 제한 구조 구현  

---

### 2. Next.js에서 사용 가능한 공통 UI 컴포넌트 가이드
- **Project Link**: [Next UI Components Guide](https://next-ui-components-guide.vercel.app/)
- **Tech & Library**: Next.js, TypeScript, React Hook Form, zustand, React Hook Form, react-day-picker, framer-motion

**What I Did**
- Next 기반 UI 컴포넌트 설계 및 가이드 페이지 구성
- controlled / uncontrolled 사용 흐름 정리
- React Hook Form 연동 예제 제공
- Field 중심 폼 레이아웃 규칙 정립
- 공통 디자인 토큰 정리
  - typography
  - spacing
  - size
  - radius / shadow
  - color / state
  - motion
 
---

### 3. 유저 대시보드
- **Project Link**: [React CRUD Project](https://react-crud-project-five.vercel.app/)
- **Tech & Library**: React, TypeScript, Vite, React Hook Form

**What I Did**
- reqres.in Mock API를 활용하여 데이터를 관리 
- fetch와 useState 기반의 CRUD 구현

---

## 🛠 Tech Stack

### Frontend
- **React, TypeScript**  
  - 컴포넌트 책임 분리 및 재사용 구조 설계  
  - 상태 변화에 따른 UI 분기 처리 경험  

- **Next.js (Page Router 학습 및 적용 경험)**  

### State & Data
- TanStack Query (서버 상태 관리 학습 및 적용)  
- React Hook Form (폼 상태 관리 및 validation 구조 설계)  

### Markup & Styling
- HTML5 (Semantic Markup, 접근성 고려)
- SCSS (BEM, 구조화된 스타일링)
- 반응형 웹 구현

### JavaScript
- ES6+, 비동기 처리(fetch)
- jQuery

### Animation & UI
- Framer Motion
- GSAP
- Swiper

### Tools
- Git, GitHub
- Figma

---

## 📂 Projects

### 1. 롯데월드 차세대 프로젝트
- **Period**: 2024.10 ~ 2025.09  
- **Tech**: React, SCSS, Framer Motion, Swiper  

**What I Did**
- 공통 컴포넌트 직접 설계 및 가이드 구축  
- Textfield, Popup 기반 역할별 컴포넌트 구조 설계  
- 상태 처리 및 이벤트 규칙을 통일하여 일관성 확보  
- Framer Motion, Swiper 기반 인터랙션 구조 정의

---

### 2. 전북은행 쏙뱅크 앱
- **Period**: 2023.09 ~ 2024.02  
- **Tech**: HTML, SCSS, JavaScript, React, Highcharts  

**What I Did**
- Highcharts 기반 데이터 시각화 UI 구현  
- 차트 옵션 및 이벤트를 구조화하여 재사용 가능한 형태로 정리  
- 데이터 가독성과 UX 개선 중심 UI 설계  

---

### 3. 삼성 프로젝트
- **Period**: 2023.07  
- **Tech**: Vue  

**What I Did**
- 기존 코드 구조 분석 후 UI 수정 및 기능 확장  
- 컴포넌트 구조 이해 기반 유지보수 수행  

---

### 4. 포스코 Flow 프로젝트
- **Period**: 2023.04 ~ 2023.06  
- **Tech**: HTML, SCSS, jQuery, IBSheet8  

**What I Did**
- 데이터 중심 UI 컴포넌트 가이드 설계  
- 반복되는 컬럼 타입, 상태 표시 등을 공통 객체로 정의  
- 개발에서 데이터와 옵션만 전달하면 확장 가능하도록 구조 설계  

---

### 5. 국민은행 빌드·배포 자동화 시스템
- **Period**: 2022.10 ~ 2023.03  
- **Tech**: React, TypeScript, Ant Design, PrimeReact  

**What I Did**
- Ant Design, PrimeReact를 Wrapper 컴포넌트로 추상화  
- 불필요한 옵션 제거 및 인터페이스 재정의  
- 커스텀 훅 기반 전역 모달 관리 구조 설계  
- 팀 내 일관된 컴포넌트 사용 기준 정립  

---

### 6. 캐논코리아 통합 프로젝트
- **Period**: 2022.03 ~ 2022.10  
- **Tech**: HTML, SCSS, jQuery  

**What I Did**
- 공통 UI 컴포넌트 가이드 설계  
- 레이아웃 및 UI 구조 표준화  
- 컴포넌트 단위로 분리하여 유지보수성 개선  

---

### 7. 하나손해보험 원데이 앱 고도화
- **Period**: 2021.07 ~ 2022.03  
- **Tech**: HTML, CSS, JavaScript, GSAP  

**What I Did**
- SVG 기반 인터랙션 구현 및 애니메이션 구조 설계  
- 상품 가입 플로우 UI 퍼블리싱  
- 인터랙션 요구사항을 구조화하여 재사용 가능한 형태로 정리  

---

## 📚 Now Learning

프론트엔드 개발자로서 구조 설계와 데이터 처리 역량을 확장하기 위해 다음 영역을 학습하고 있습니다.

- Next.js App Router  
- TanStack Query 기반 서버 상태 관리
- React Suspense

---

## 📫 Contact

- **GitHub**: https://github.com/CHANSIK-CHOI  
- **Email**: ccsik0828@gmail.com  
- **Blog**: https://velog.io/@ckstlr0828/posts
