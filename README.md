...
### 🔹 주요 업무
- 대기업 위주 구축 프로젝트에서 퍼블리싱 업무 담당
- 공통 컴포넌트, 가이드 설계를 통한 재사용성과 유지보수성 개선
- 페이지 제작, 인터렉션 구현 등

### 🔹 리더십
- 퍼블리싱 파트장 (2명 리딩)  
- UI 컴포넌트 가이드 및 협업 기준 정의  
- 코드 리뷰 및 프로젝트 진행 관리  
...
### 🔹 Frontend
- **React, TypeScript**  
  - 컴포넌트 책임 분리 및 재사용 구조 설계 (공통 컴포넌트 가이드)
  - React 환경에서 퍼블리싱, 인터렉션 구현, 상태 변화에 따른 UI 분기 처리 경험
...
**주요 작업**
- 데이터 중심 UI 컴포넌트 가이드 설계하여 반복되는 컬럼 타입, 상태 표시 등을 공통 객체로 정의
...
**주요 작업**
- Ant Design, PrimeReact 그리드 시스템을 Wrapper 컴포넌트로 추상화하여 불필요한 옵션 제거 및 인터페이스 재정의  
- 공통 컴포넌트 설계 및 가이드 구축
...
- **프로젝트 소개**:
  - 사용자가 작성한 피드백 콘텐츠를 관리자 검수 후 공개하고, 공개 이후 댓글과 알림으로 상호작용이 이어지도록 설계한 콘텐츠형 커뮤니티 보드입니다. Next.js Page Router와 Supabase를 기반으로 인증, 권한, 상태 전이, 댓글, 실시간 알림, API Routes를 구현했습니다.
  - 단순 CRUD가 아니라 작성 -≫ 검수 -≫ 공개 -≫ 댓글/답글 -≫ 알림 -≫ 재방문으로 이어지는 사용자 행동 흐름을 기준으로 UI와 API 구조를 재설계했습니다.

### 🔹 2. Claude Code 기반 AI 협업형 Vue UI 컴포넌트 제작 시스템
- **프로젝트 링크**: [claude-code-agent-ui-components-guide-vue](https://chansik-choi.github.io/claude-code-agent-ui-components-guide-vue/guide/)
- **기술 & 라이브러리**: Nuxt, Vue , TypeScript, SCSS, Radix-Vue, Claude Code
- **프로젝트 소개**: Claude Code를 단순 코드 생성 도구로 사용하지 않고, 기획, 구현, QA, 리뷰 역할을 Agent 단위로 분리해 반복적인 UI 컴포넌트 제작 과정을 자동화한 프로젝트입니다. Figma MCP, Context7 MCP, Playwright MCP를 활용해 디자인 토큰 추출, 라이브러리 API 확인, 화면 검증까지 하나의 워크플로우로 연결했습니다.
...
프론트엔드 개발자로서 구조 설계와 데이터 처리 역량을 확장하기 위해 다음 영역을 학습하고 있습니다.
